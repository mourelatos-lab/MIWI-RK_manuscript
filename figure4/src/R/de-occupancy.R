#!/usr/bin/env Rscript
#
# Calculating some basic stats and RNA-Seq DE and Ribo-Seq/Degradome-Seq occupancy
# Because we don't have replicates, edgeR is being used with exactTest for RNA-Seq with bcv=0.4
# Ribo-Seq occupancy is calculated with glmLRT because exactTest cannot do formulas in edgeR, and with reduced formula in DESeq2
# For exactTest we would have to manually divide riboseq/rnaseq (or degradomseq/rnaseq) and then use this for calc. but
#   I am not sure edgeR would like it - (Ribo_A1/mRNA_A1)/(Ribo_M1/mRNA_M1)
#
# IMPORTANT: This follows tximport normalization rather than the traditional normalization more common to usual counts
#
################################################################################
...
library(ggcorrplot) # Loaded later on because of conflicts with biomaRt
...
if ("help" %in% names(options.args)) {
  cat("Usage:
    ...
    --samples mmu.RNASeq.total.MiwiHet.P24.1 mmu.RNASeq.total.MiwiHet.P24.2 mmu.RNASeq.total.MiwiHet.P24.3 mmu.RNASeq.total.MiwiRK.P24.1 mmu.RNASeq.total.MiwiRK.P24.2 mmu.RNASeq.total.MiwiRK.P24.3 \
        mmu.PARESeq.polya.MiwiHet.P24.1 mmu.PARESeq.polya.MiwiHet.P24.2 mmu.PARESeq.polya.MiwiRK.P24.1 mmu.PARESeq.polya.MiwiRK.P24.2 \
...
genes_to_viz <- options.args$genes_to_viz # Comma-separate list of genes to visualize (=name) in 'opposite' plots; please use values in either gene_name or gene_id, do not mix (not implemented)
...
if (!file.exists(paste(resdir, "data", "mart-desc.tsv", sep = "/"))) {
  ...
  library(biomaRt)
  # This will work ok for human and mouse. For other organisms please change gene_name varible specific for that organism
  # Get Ensembl host
...
# We got a LOT of missing transcripts if we do GTF to map and biomaRt to tx2gene; better to do it from gtf right away
tx2gene <- desc %>%
...
################################ VERY ROUGH BETA ###############################
## Differential ribosome/degradome loading analysis (translational/degradation efficiency)
...
keep <- filterByExpr(y, group = coldata$condition) # edgeR mabual specifies we should not use "baseline" coldata for this filtering but only the "difference" condition
...
# We have to rely on the last column as comparison for the statistical test
...
pdf(paste(resdir, "de-plot.pdf", sep = "/"))
  ...
  plot(et.adj$table$logCPM, et.adj$table$logFC,
    ...
    #    main = paste(et.adj$comparison[2], "vs", et.adj$comparison[1]))
