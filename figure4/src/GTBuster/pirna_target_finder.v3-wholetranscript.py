...
# SEED_START = 1 and SEED_END = 7 mean 2-7 as the seed
SEED_START = 1
SEED_END = 7
SEED_LEN = SEED_END - SEED_START

## g8-21 as the non-seed region # original value
## g8-26 as the non-seed region
NONSEED_START = SEED_END
#NONSEED_END = 21 # original value
NONSEED_END = 26
...
parser.add_argument("--min-match", type=int, default=6,
                    help="Minimum number of non-seed matches required in g8-25. \
                    This is to prevent the output from being huge")
parser.add_argument("--distance-from-canonical-cut-site", type=int, default=2,
                    help="DISABLED! With the default value of 2, the script will output \
...
pirna_insert = args.pirna_insert
        ...
        if pot_t_seed_rc in seed2pirna:
            for pirna in seed2pirna[pot_t_seed_rc]:
                ...
                if len(pot_t_nonseed_rc) == 0:
                    ...
                    pot_t_nonseed_rc = "N"
#                if gt_n_matches_nonseed >= min_match and abs(j+SEED_LEN-1-58) <= distance_from_canonical: # original for 101 bp long sequences
                if gt_n_matches_nonseed >= min_match: # original for 101 bp long sequences
