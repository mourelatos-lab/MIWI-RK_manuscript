#!/usr/bin/env bash
#
# Original script had matches from g2-21 (20 nt). We changed it to g2-26 (25 nt) to be more strict.
#
...
## Just a hack. Rememeber to make sure to split the fasta file into fewer than 100 parts
#for i in {00..99}; do
...
#    if [ -f ${deg_prefix}${i}.fa ]; then
	...
	if [ ${RUN_RNAFOLD} = true ]; then
	    echo "python src/GTBuster/pirna_target_finder.v3.py -p ${input} -s ${i} --distance-from-canonical-cut-site $dist_from_cann > ${para_prefix}.gt.${name} && \
...
"
	...
	## Without RNAfold
	    ## echo "python pirna_target_finder.v3.py -p ${input} -s ${deg_prefix}${i} > ${para_prefix}.gt.${i}"
	    echo "python src/GTBuster/pirna_target_finder.v3.py -p ${input} -s ${i} --distance-from-canonical-cut-site $dist_from_cann > ${para_prefix}.gt.${name}"
#	fi
    fi
done > run/cmds.${R}.txt
